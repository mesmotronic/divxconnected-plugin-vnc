...
package connectedvnc.rfb
{
	import com.darronschall.utils.StringUtil;
	
	import connectedvnc.errors.ConnectionError;
	import connectedvnc.events.*;
	import connectedvnc.rfb.constants.*;
	...
	public class RFBProtocol extends Socket {
		 ...
		 * to the server.
		 */
		protected var rightClicker:uint=5;
		public function writePointerEvent(event:MouseEvent, rightButtonDown:Boolean=false):void
		{
			// Let the server know what kind of message is coming from the client
			...
			if (rightButtonDown) pointerMask |= 0x05;
			else if (event.buttonDown) pointerMask = 1;
			
			// TODO: What do we do about middle and right mouse button?
			
			// scroll down - button "5"
			if (event.delta < 0) pointerMask |= 0x10;
				// scroll up - button "4"
			else if (event.delta > 0) pointerMask |= 0x04;
